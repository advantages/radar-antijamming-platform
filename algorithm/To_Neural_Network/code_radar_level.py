...
save_path = os.path.join("freq50_result_1203_small/", jammertype, str(int(iteration)))
cuda_num=args.cuda
# Plot iters
...
for i in iters:
    ...
    rew, rew_vec,_, a_jammer, jammer_strategy = game.step_BLN(a_radar, jammertype)

    rewards += rew
    # Policy update
    if alg == 'NN':
        algo1.update_policy(a_jammer,jammer_strategy)  # Not really use a_radar
    elif alg == 'Exp3R':
        u_normal = rew-3
    ...
    # Calculate Regret (per-iteration regret)
    s_vec = rew_vec-rew_vec[a_radar]
    S_value = S_value + s_vec.max()
    regret = 1 / T * S_value
