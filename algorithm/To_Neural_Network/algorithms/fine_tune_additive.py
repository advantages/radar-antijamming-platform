...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # self.q_linear=nn.Linear(self.n_embd,self.n_embd)
        # self.v_linear=nn.Linear(self.n_embd,self.n_embd)
        self.q_linear=nn.Parameter(torch.ones(1,1,self.n_embd,requires_grad=True))
        self.v_linear=nn.Parameter(torch.ones(1,1,self.n_embd,requires_grad=True))
    ...
    def forward(self, x):
        ...
        q=q.reshape(B,T,C)
        v=v.reshape(B,T,C)
        q=(self.q_linear*q).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)
        v=(self.v_linear*v).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.mid_linear=nn.Linear(4 * config.n_embd,4 * config.n_embd)
        self.mid_linear=nn.Parameter(torch.ones(1,1,4 * config.n_embd,requires_grad=True))
    def forward(self, x):
        x = self.c_fc(x)
        x = self.nonlin(x)
        x=self.mid_linear*x
...
class GPT_FT(nn.Module):
    def __init__(self, config):
        ...
        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False) # final layer， map back to vocabulary size, to form a_radar prob density
